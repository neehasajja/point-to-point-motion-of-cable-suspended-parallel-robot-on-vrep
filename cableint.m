# here we estabilish the connection between matlab and vrep.
...
if (clientID>-1)
  ...
  #here the positions are only set in Y direction as the motion is only in this direction.
        %set Y_initial
  ...
  # these are the constants to work on the formula
...
#here the motion is performed in one direction
#again when we want the robot to travel back from final to initial positions , its just by replacing initial to final and final to initial and again using the same controller.
%command to robot
